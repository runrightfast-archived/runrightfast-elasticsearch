...
(function() {
	 ...
	 * </code>
	 */
	EntityDatabase.prototype.newRangeQuery = function(params) {
		var query = this.ejs.RangeQuery(params.field);
	...
	var findByFieldParamsSchema = {
		...
		value : joi.types.Object().without('range')
	};
	...
	EntityDatabase.prototype.findByField = function(params) {
		...
		return when.promise(function(resolve, reject) {
			...
			if (!lodash.isUndefined(params.value)) {
				request.query(self.ejs.TermQuery(params.field, params.value));
			} else {
				request.query(self.newRangeQuery(params));
			}
