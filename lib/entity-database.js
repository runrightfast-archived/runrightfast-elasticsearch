...
(function() {
	 ...
	 * 
	 * <code>
	 * field			OPTIONAL - field name - if none specified, then all entities are searched
	 * value			OPTIONAL - The field value to match on. Ignored if 'field' is not set. 
	 ...
	 * from				OPTIONAL - A search result set could be very large (think Google). 
	 *                             Setting the from parameter allows you to page through the result set by making multiple request. 
	 *                             This parameters specifies the starting result/document number point. Combine with 'pageSize' to achieve paging.
	 ...
	 * sort.desc		OPTIOANL - If set to true, then sort order is descending - by default it is ascending
	 * </code>
	 */
	EntityDatabase.prototype.getEntitiesByField = function(params) {
		// TODO: implement
	...
	EntityDatabase.prototype.getEntitiesByCreatedOn = function(params) {
		if (lodash.isObject(params)) {
			params.field = 'createdOn';
			return this.getEntitiesByFieldRange(params);
		}
		return when.reject(new Error('params is required'));
	};
