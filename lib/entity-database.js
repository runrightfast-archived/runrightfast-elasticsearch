...
(function() {
	 ...
	 * from				OPTIONAL - The lower bound. Defaults to start from the first.
	 * to				OPTIONAL - The upper bound. Defaults to unbounded.
	 ...
	 * pageSize			OPTIONAL - Sets the number of results/documents to be returned. This is set on a per page basis.
	 * sort.field		OPTIONAL - Sets the sort field for the query
	 * sort.descending	OPTIOANL - If set to true, then sort order is descending - by default it is ascending
	 * </code>
	 */
	EntityDatabase.prototype.getEntitiesByFieldRange = function(params) {
		var getEntitiesByFieldRangeParamsScehma = {
		...
		var self = this;
		return when.promise(function(resolve, reject) {
			var err = joi.validate(params, getEntitiesByFieldRangeParamsScehma);
			if (err) {
				reject(err);
				return;
			}
			...
			var matchQuery = self.ejs.MatchQuery(params.field);
			matchQuery.slop(0);
			matchQuery.type('phrase');
			request.query(matchQuery);

			if (!lodash.isUndefined(params.timeout)) {
				request.timeout(params.timeout);
			}
			if (!lodash.isUndefined(params.returnFields)) {
				request.fields(params.returnFields);
			}
			if (!lodash.isUndefined(params.version)) {
				request.version(params.version);
			}
			if (!lodash.isUndefined(params.from)) {
				request.from(params.from);
			}
			if (!lodash.isUndefined(params.to)) {
				request.to(params.to);
			}
			if (!lodash.isUndefined(params.includeLower)) {
				request.includeLower(params.includeLower);
			}
			if (!lodash.isUndefined(params.includeUpper)) {
				request.includeUpper(params.includeUpper);
			}
			if (!lodash.isUndefined(params.pageSize)) {
				request.size(params.pageSize);
			}
			if (!lodash.isUndefined(params.sort)) {
				request.sort(params.sort.field, !!params.sort.descending);
			}
	...
	EntityDatabase.prototype.findAll = function(params) {
		...
		if (log.isDebugEnabled()) {
			log.debug('getEntitiesByCreatedOn() : params : ' + JSON.stringify(searchParams, undefined, 2));
		}
		...
		return when.promise(function(resolve, reject) {
			...
			if (!lodash.isUndefined(searchParams.to)) {
				request.to(searchParams.to);
			}
			if (!lodash.isUndefined(searchParams.includeLower)) {
				request.includeLower(searchParams.includeLower);
			}
			if (!lodash.isUndefined(searchParams.includeUpper)) {
				request.includeUpper(searchParams.includeUpper);
			}
			...
			if (log.isDebugEnabled()) {
				log.debug('findAll() : request : ' + request);
			}
	 ...
	 * pageSize			OPTIONAL - Sets the number of results/documents to be returned. This is set on a per page basis.
	 * sort.field		OPTIONAL - Sets the sort field for the query
	 * sort.descending	OPTIONAL - If set to true, then sort order is descending - by default it is ascending
	 * </code>
	 */
	EntityDatabase.prototype.findAllSortedByCreatedOn = function(params) {
		findAll(params);
	};
	...
	EntityDatabase.prototype.getEntitiesByUpdatedOn = function(params) {
		if (lodash.isObject(params)) {
			...
			return this.getEntitiesByFieldRange(params);
		}
		return this.getEntitiesByFieldRange({
			field : 'updatedOn',
			sort : {
				field : 'updatedOn',
				descending : true
