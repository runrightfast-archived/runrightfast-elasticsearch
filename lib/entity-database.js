...
(function() {
	 ...
	 * </code>
	 */
	EntityDatabase.prototype.setEntity = function(params,refresh) {
		var self = this;
		return when.promise(function(resolve, reject) {
			...
			extend(newEntity,params.entity);
	 ...
	 * sort:{					
	 *   field			OPTIONAL - Sets the sort field for the filter 
	 *   descending		OPTIONAL - If set to true, then sort order is descending
	 * }
	 * </code>
	 * 
	 * @param {Object}schema
	...
	EntityDatabase.prototype.newSearchRequest = function(searchParams, schema, filter) {
		...
		if (!lodash.isUndefined(searchParams.multiFieldSort)) {
			searchParams.multiFieldSort.forEach(function(sort){
		...
		if(searchParams.facet){
			...
			termsFacet.facetFilter(filter);
			request.facet(termsFacet);			
			if(searchParams.facet.field){
				termsFacet.field(searchParams.facet.field);
			}else{
				termsFacet.fields(searchParams.facet.fields);
			}

			if (!lodash.isUndefined(searchParams.facet.size)) {
				termsFacet.size(searchParams.facet.size);
			}

			if (!lodash.isUndefined(searchParams.facet.excludes)) {
				termsFacet.excludes(searchParams.facet.excludes);
			}

			if (!lodash.isUndefined(searchParams.facet.order)) {
				termsFacet.order(searchParams.facet.order);
			}

			if (!lodash.isUndefined(searchParams.facet.regex)) {
				termsFacet.regex(searchParams.facet.regex);

				if (!lodash.isUndefined(searchParams.facet.regexFlags)) {
					termsFacet.regexFlags(searchParams.facet.regexFlags);
				}
			}
		}
	...
	var findByFieldParamsSchema = {
		...
		}).without('multiFieldSort'),
		multiFieldSort : joi.types.Array().includes(joi.types.Object({
			field : joi.types.String().required(),
			descending : joi.types.Boolean()
		})).without('sort'),
		// RangeFilter settings
		field : joi.types.String().required(),
		...
		facet: joi.types.Object({
			name : joi.types.String().required(),
			field: joi.types.String().without('fields'),
			fields: joi.types.Array().includes(joi.types.String()).without('field'),
			size : joi.types.Number().min(1),
			excludes : joi.types.Array().includes(joi.types.String()),
			order : joi.types.String().valid('count','term','reverse_count','reverse_term'),
			regex : joi.types.String(),
			regexFlags : joi.types.String().with('regex')
		})
	};
	 ...
	 * sort:{					
	 *   field			OPTIONAL - Sets the sort field for the filter 
	 *   descending		OPTIONAL - If set to true, then sort order is descending - by default it is descending = false
	 * }
	 * mulutFieldSort:[
	 *  {					
	 *   field			OPTIONAL - Sets the sort field for the filter 
	 *   descending		OPTIONAL - If set to true, then sort order is descending - default is descending = false
	 *  }
	 * ]
	 ...
	 * }
	 * facet:{			
	 *   name 			REQUIRED - The name which be used to refer to this facet.
	 *   field 			MUTUAL - String - Sets the field to be used to construct the this facet. Set to _index to return a facet count of hits per _index the search was executed on.
	 *						   - either 'field' or 'fields' is required
	 *   fields 		MUTUAL - Array - Aggregate statistical info across a set of fields.
	 *						   - either 'field' or 'fields' is required
	 *	 size 			OPTIONAL - Sets the number of facet entries that will be returned for this facet. For instance, you might ask for only the top 5 authors although there might be hundreds of unique authors.
	 *   excludes		OPTIONAL - Allows you to filter out unwanted facet entries. When passed a single term, it is appended to the list of currently excluded terms. If passed an array, it overwrites all existing values.
	 *   order 			OPTIONAL - Sets the type of ordering that will be performed on the date buckets. Valid values are: 
     *	 								
	 ...
	 *   regex			OPTIONAL - Allows you to only include facet entries matching a specified regular expression.
	 *   regexFlags		OPTIONAL - Allows you to set the regular expression flags to be used with the regex
	 * }
	 * </code>
	 */
	EntityDatabase.prototype.findByField = function(params) {
		if(log.isDebugEnabled()){
			log.debug('findByField() : params : ' + JSON.stringify(params,undefined,2));
		}		

		var self = this;
		var createFilter = function(){
			if (!lodash.isUndefined(params.value)) {
				return self.ejs.TermFilter(params.field, params.value);
			} 
			return self.newRangeFilter(params);			
		};
		...
		return when.promise(function(resolve, reject) {			
			...
			var request = self.newSearchRequest(searchParams, findByFieldParamsSchema,filter);	
			request.filter(filter);					
			if(log.isDebugEnabled()){
				log.debug('findByField() : request : ' + request);
			}
	 ...
	 * pageSize			OPTIONAL - Number - Sets the number of results/documents to be returned. This is set on a per page basis.
	 * sort:{					
	 *   field			OPTIONAL - String - Sets the sort field for the filter - default is 'updatedOn'
	...
	EntityDatabase.prototype.findAll = function(params) {
		...
		return when.promise(function(resolve, reject) {
			...
			request.filter(self.ejs.MatchAllFilter());
			request.doSearch(checkElasticsearchResult.bind(null, resolve, reject), reject);
		});
